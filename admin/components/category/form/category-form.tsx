...
const CategoryForm = ({ initialData }: CategoryFormProps) => {
  ...
  const description = initialData ? 'Edit category details' : 'Create new category';
  ...
  const onSubmit = async (values: CategoryFormPropsValues) => {
    try {
      setLoading(true);

      if (initialData) {
        await axios.patch(`/api/${params.storeId}/categories/${initialData.id}`, values);
      } else {
  ...
  const onDelete = async () => {
    try {
      ...
      toast.error('Make sure you removed all products before deleting the category');
    } finally {
      setOpen(false);
      setLoading(false);
    }
